"""H11 layer 3: multi-size particle-population COMSOL skeleton.

This layer extends the solve-locked external-plume model with the particle
physics that are structurally required by the A-group DPV data.  It does not
pretend that the detected DPV diameter distribution is the feedstock PSD.
Instead, the particle-diameter degree of freedom is enabled and three
lognormal quadrature values are declared around a provisional median/GSD
solely to expose the population pathways in the COMSOL tree.  Their release
weights are intentionally undefined.

The model includes:

* Newtonian Lagrangian particle tracing with Schiller-Naumann drag;
* particle temperature as an explicit degree of freedom;
* convective and radiative particle heat exchange;
* accumulated momentum/heat feedback variables for later two-way coupling;
* a fixed internal DPV observation cross-section in the measurement branch.

Particle release remains absent because injector position, angle, velocity,
and the traceable feedstock PSD are not recorded.  Therefore the transient
study remains locked and no simulated DPV value is produced.
"""
...
class ParticleEntryContract:
    """Solve gates and explicit prior status for particle-population physics."""
    ...
    def size_sensitivity_anchors_um(self) -> dict[str, float]:
        """Return the confirmed bounds and their neutral geometric center."""

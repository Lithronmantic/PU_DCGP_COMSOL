"""H11 layer 9: conservative compressible target-impact COMSOL skeleton.

The previous Single-Phase Flow + Heat Transfer construction produced bounded
solutions and good mass closure, but its official COMSOL total-energy residual
did not decrease under refinement.  This independent branch uses COMSOL's
predefined High Mach Number Flow, k-omega interface, whose dependent variables
are pressure, velocity, temperature, k, and omega and whose kinetic-energy
option solves the conservative total-energy equation.

The interface name does not imply that the entire plume is supersonic.  It is
selected because the provisional centerline exit state is near Mach 0.4 and
because the all-Mach compressible formulation provides thermodynamically
consistent pressure work and total-energy transport.

The effective exit state remains an uncalibrated, training-only placeholder.
This file builds and audits the physics tree but does not solve it.
"""
...
class ConservativeTargetContract:
    """Traceable settings for the conservative compressible model tree."""

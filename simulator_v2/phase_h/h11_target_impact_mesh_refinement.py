"""H11 layer 8: refine a converged target-impact solution onto a finer mesh.

The nonlinear target-plume equations are solved on every target mesh.  A
bounded solution from the immediately coarser mesh is used only as the
initial iterate, which is standard continuation in mesh size.  The target
solution is accepted only after a new stationary solve at the frozen
full-load physics and a fresh conservation audit.

This runner does not calibrate the effective nozzle state and cannot unlock
experimental or paper predictions by itself.
"""
...
class MeshRefinementContract:
    """Numerical contract for one adjacent level in the mesh ladder."""
...
def configure_projected_refinement(
    ...
    *,
    ...
    """Build a stationary target-mesh solve from a coarser bounded solution."""

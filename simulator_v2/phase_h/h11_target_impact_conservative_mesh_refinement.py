"""H11 layer 11c: adjacent-mesh refinement of the final all-Mach RANS model.

The converged immediately coarser all-Mach solution is projected only as the
initial iterate.  COMSOL then resolves the unchanged conservative k-omega
equations on the target mesh and recomputes all conservation metrics.
"""
...
class ConservativeMeshRefinementContract:
    """Numerical contract for one conservative adjacent-mesh step."""
...
def configure_refinement(
    ...
    *,
    ...
    # The coarse restart model contains withsol() expressions that refer to
    # its original weakly-compressible seed.  Those expressions are no longer
    # needed and can be undefined after remeshing.  The target study below
    # explicitly projects the converged all-Mach source solution instead.

"""H11 layer 6: reproducible target-impact numerical smoke solve.

This runner verifies that the corrected target branch can reach its nominal
effective inlet state without changing the physical model.  It uses COMSOL's
parametric continuation from an ambient, low-speed seed to the nominal
boundary.  The result is a numerical and conservation diagnostic only:
the effective inlet remains uncalibrated and therefore cannot be used as an
experimental prediction or as a paper result.
"""
...
class NominalSmokeContract:
    """Numerical continuation settings, not experimental calibration values."""
...
def configure_continuation(jm: Any, contract: NominalSmokeContract) -> None:
    """Apply the verified weakly-compressible formulation and load path."""
...
def configure_full_load_refinement(jm: Any, contract: NominalSmokeContract) -> str:
    """Create a tight full-load solve initialized from the continuation endpoint."""

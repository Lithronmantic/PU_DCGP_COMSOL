"""Pre-registered DOE pathways for joint effective-exit calibration.

The A-group DPV geometry contains no workpiece and keeps the observation plane
at 100 mm in the gun frame.  Therefore the four executed DOE factors do not
all enter the same COMSOL equation:

* current and argon may parameterize the unobserved effective exit state;
* powder feed enters particle release/loading and, if justified, two-way
  coupling, not the gas exit boundary;
* spray distance has no geometric or boundary role in the fixed-DPV free jet
  and is retained only by the empirical discrepancy/observation layer;
* fixed H2 divided by argon is a deterministic transform of argon and is not
  added as a second independent predictor.

This module freezes those pathways and standardized design coordinates.  It
does not estimate coefficients before the particle and DPV operators exist.
"""
...
class AGroupDOEPoint:
    """One executed four-factor setting, retaining the original units."""
...
class EffectiveExitParameterizationContract:
    """Frozen factor ranges, pathways, and calibration claim boundary."""
...
def pathway_design(
    ...
    """Return pre-registered design bases without estimating coefficients."""

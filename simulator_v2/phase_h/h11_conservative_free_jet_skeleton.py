"""H11: conservative free-jet skeleton for the fixed A-group DPV plane.

This branch is deliberately separate from the workpiece-impact branch:

* the coordinate system is attached to the spray gun;
* the DPV observation cross-section is fixed at 100 mm from the gun;
* no workpiece or gun-to-workpiece spray-distance variable is present;
* the annulus outside the effective nozzle is a stationary, adiabatic torch
  face rather than an artificial ambient inlet;
* only the radial and downstream far fields are pressure-temperature openings;
* mass, momentum, and total energy use COMSOL's all-Mach conservative
  turbulent-flow interface.

The pure-argon material and the 10,000 K / 600 m/s effective exit are numerical
reference settings, not experimental truth.  The Ar DOE flow and the reported
H2 mass flow are not on a compatible composition basis, so an Ar-H2 mixture is
not silently constructed.  This file builds and audits a solve-locked COMSOL
tree.  It does not generate paper predictions.
"""
...
class ConservativeFreeJetContract:
    """Frozen geometry and claim boundary for the A-group gas-phase skeleton."""
...
def validate_boundary_role_sets(
    ...
    """Validate disjoint physical boundaries and the internal DPV section."""

"""H11 layer 11k: comparable-observable radial-domain independence audit.

The original v1 contract compared heat integrated over the complete target
boundary.  Expanding the radial fluid domain also lengthens that boundary, so
the v1 quantity changes its physical integration area between models and is
not a valid domain-independence estimand.  Version 2 retains the original
1 percent heat-change limit but applies it to the fixed 0--30 mm target ROI
that is common to all three models.  Full-boundary heat remains visible only
as a non-gating diagnostic.

The 30 mm ROI is the already frozen maximum radius of the common field grid
and was selected before evaluating any ROI heat result.  No geometry, field,
solution, threshold, or data row is altered by this audit.
"""
...
class DomainIndependenceContract:
    """Paper-facing numerical-domain comparison on common observables."""
...
def _flat_scalar(value: object) -> float:
    return float(value.reshape(-1)[0])  # type: ignore[attr-defined]
...
def target_heat_expressions(
    ...
    """Return comparable ROI heat and the legacy full-boundary diagnostic."""
...
def evaluate_domain_model(
    ...
    *,
    ...
    """Evaluate one accepted model on the pre-registered common observables."""

    contract.validate()
    if radius_mm not in contract.radii_mm:
        raise ValueError("Model radius is outside the frozen domain audit")
    dataset = list(model / "datasets")[-1]  # type: ignore[operator]
    selector: dict[str, object] = {}
    inner_indices, _ = model.inner(dataset)  # type: ignore[attr-defined]
    ...
    for name, expression, unit in (
        ...
        values = model.evaluate(  # type: ignore[attr-defined]
            expressions,
    ...
    outflow, inflow, opening_area = model.evaluate(  # type: ignore[attr-defined]
        [
    ...
    fixed_roi_heat, full_boundary_heat = model.evaluate(  # type: ignore[attr-defined]
        [
...
def adjacent_pair_comparison(
    ...
    return {
        ...
        "near_target_temperature_change_fraction": relative_scalar_change(
            float(source_near["temperature_k"]),  # type: ignore[index]
            float(target_near["temperature_k"]),  # type: ignore[index]
        ),
        "near_target_speed_change_fraction": relative_scalar_change(
            float(source_near["speed_m_s"]),  # type: ignore[index]
            float(target_near["speed_m_s"]),  # type: ignore[index]
        ),
        "target_fixed_roi_heat_change_fraction": relative_scalar_change(
            float(source_heat["fixed_roi"]),  # type: ignore[index]
            float(target_heat["fixed_roi"]),  # type: ignore[index]
        ),
        ...
        "common_field_changes": {
            "temperature_anomaly_normalized_l2": normalized_l2_change(
                source_fields["temperature_k"],  # type: ignore[index]
                target_fields["temperature_k"],  # type: ignore[index]
                offset=300.0,
            ),
            "speed_normalized_l2": normalized_l2_change(
                source_fields["speed_m_s"],  # type: ignore[index]
                target_fields["speed_m_s"],  # type: ignore[index]
            ),
            "pressure_l2_over_ambient": normalized_l2_change(
                source_fields["absolute_pressure_pa"],  # type: ignore[index]
                target_fields["absolute_pressure_pa"],  # type: ignore[index]
                absolute_scale=101_325.0,
            ),
        },
        "radial_opening_two_way_activity_change_fraction": (
            relative_scalar_change(
                float(source_activity["two_way_total"]),  # type: ignore[index]
                float(target_activity["two_way_total"]),  # type: ignore[index]
            )
        ),
...
def finalize_domain_independence(
    ...
    outer_limits = {
        ...
        "common_temperature": (
            outer_fields["temperature_anomaly_normalized_l2"],  # type: ignore[index]
            contract.common_temperature_anomaly_l2_limit_fraction,
        ),
        "common_speed": (
            outer_fields["speed_normalized_l2"],  # type: ignore[index]
            contract.common_speed_l2_limit_fraction,
        ),
        "common_pressure": (
            outer_fields["pressure_l2_over_ambient"],  # type: ignore[index]
    ...
    contraction_pairs = {
        ...
        "common_temperature": (
            inner_fields["temperature_anomaly_normalized_l2"],  # type: ignore[index]
            outer_fields["temperature_anomaly_normalized_l2"],  # type: ignore[index]
        ),
        "common_speed": (
            inner_fields["speed_normalized_l2"],  # type: ignore[index]
            outer_fields["speed_normalized_l2"],  # type: ignore[index]
        ),
        "common_pressure": (
            inner_fields["pressure_l2_over_ambient"],  # type: ignore[index]
            outer_fields["pressure_l2_over_ambient"],  # type: ignore[index]
        ),
    }
    contraction_gates = {

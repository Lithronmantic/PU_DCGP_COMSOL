"""H11 layer 4: workpiece-impact geometry separated from DPV measurement.

Two experimental geometries are now explicit and must never be collapsed:

``measurement branch``
    No workpiece is present.  The DPV optical sampling plane is located at a
    prescribed gun-to-DPV distance (90/100/110 mm depending on protocol).

``impact branch``
    A workpiece is present at the prescribed gun-to-workpiece spray distance.
    The target wall is at ``z=d_spray`` and no DPV plane is part of this
    geometry.

This file builds the impact branch only.  The previously generated
gun-attached fixed-DPV model remains the measurement branch.  The separation
removes the apparent ``d_spray < z_DPV`` contradiction because the two
distances no longer coexist in one physical domain.
"""
...
class DualBranchGeometryContract:
    """Operator-confirmed distinction between measurement and impact setups."""
...
def build_model(client: Any, contract: DualBranchGeometryContract) -> tuple[Any, Any]:
    """Build the workpiece-impact geometry; no DPV selection is created."""

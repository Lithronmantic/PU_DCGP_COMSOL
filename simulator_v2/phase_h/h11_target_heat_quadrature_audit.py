"""H11 layer 11m: audit fixed-ROI target heat quadrature convergence.

The retained 60/80/100 mm continuation fails only because the 80-to-100 mm
change in ``int_target_hmnf(if(r<=30 mm,...))`` does not contract.  The
indicator cuts target-boundary elements and the integration coupling uses
finite-order Gauss quadrature.  This postprocessing-only audit is frozen
before changing the integration order or reading any ordered result.

The solved fields are never modified or saved.  The existing target
integration operator is evaluated at orders 4, 8, 12, and 16, then restored
to its original setting.  A heat comparison is admissible only if every
model changes by at most 0.1 percent between orders 12 and 16.  The original
1 percent domain-change limit and outer-pair contraction requirement remain
unchanged.
"""
...
class TargetHeatQuadratureContract:
    """Frozen postprocessing-only quadrature audit."""
...
def ordered_target_heat(
    ...
    """Evaluate the same solved field at frozen integration orders."""

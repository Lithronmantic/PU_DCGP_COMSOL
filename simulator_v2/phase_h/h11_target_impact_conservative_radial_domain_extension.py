"""H11 layer 11j: extend the radial far field at fixed physical mesh controls.

The accepted 40 mm gas-phase model still shows appreciable two-way activity
at the radial pressure opening.  This runner changes only ``r_domain`` and
rebuilds the same two-domain geometry and mesh with the same absolute global
and local size settings.  Named selections are re-evaluated and the
torch-face, target, nozzle, and radial-open boundary assignments are audited
before a new stationary solve.

The source solution is an initial iterate only.  The target-radius equations
are reassembled and solved.  This is numerical domain verification, not
calibration or a paper prediction.
"""
...
class RadialDomainExtensionContract:
    """Frozen definition of one outward radial-domain step."""
...
def controlled_initial_expressions(
    ...
    """Return a finite cross-geometry initial field without extrapolation.

    COMSOL's direct solution initialization maps zero outside the source
    mesh.  That is inadmissible for compressible flow because it creates
    zero absolute pressure, temperature, and density in the newly added
    annulus.  ``if`` is pointwise lazy in COMSOL, so ``withsol`` is called
    only where the old mesh is defined.  A 0.01 mm inward buffer keeps the
    interpolation search away from the old mesh boundary without altering
    a resolved physical length scale.
    """
    ...
    return {
        ...
        # The accepted all-Mach solution stores absolute pressure in ``p``.
...
def apply_controlled_initialization(
    ...
    *,
    ...
    """Install source-inside/ambient-outside physics initial expressions."""
...
def rebuild_radial_domain(
    ...
    """Rebuild geometry, selections, boundary assignments, and fixed mesh."""
    ...
    # ``size1`` was physics-controlled at the 40 mm source radius.  Its
    # evaluated hmax/hmin otherwise scale with the enlarged bounding box.
    # Freeze those evaluated physical values before rebuilding the target
    # mesh so the domain study does not silently coarsen the target region.
...
def probe_controlled_initialization(
    ...
    *,
    ...
    """Compile only the target initial vector and audit both radial regions."""

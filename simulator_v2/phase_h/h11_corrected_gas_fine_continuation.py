"""Solve the corrected gas case independently on the true level-2 mesh."""
...
MODEL_PATH = MODEL_DIR / "corrected_t11160_u1090_mesh_level2_continuation.mph"
AUDIT_PATH = OUTPUT_DIR / "h11_corrected_gas_fine_continuation_audit.json"
LOG_PATH = OUTPUT_DIR / "h11_corrected_gas_fine_continuation.log"
...
def solve(client: Any) -> dict[str, Any]:
    ...
    try:
        ...
        solver = configure_studies(jm, base)
        ...
        if mesh_audit["elements"] < contract.minimum_fine_elements:
            raise RuntimeError("The true level-2 mesh did not exceed the bridge")
        print(
            f"fine mesh built: {mesh_audit['elements']} elements; "
            "starting frozen load continuation",
            flush=True,
        )
        continuation_started = time.time()
        jm.study("std1").run()
        continuation_runtime = time.time() - continuation_started
        print("fine-mesh load continuation complete", flush=True)
        refinement_started = time.time()
        jm.study("std_refine").run()
        refinement_runtime = time.time() - refinement_started
        print("fine-mesh 1e-6 refinement complete", flush=True)
        metrics = evaluate_solution(model, base)
        gates = _gas_gates(metrics)
        domain = _domain_parameters(jm)
        gates.update(
            {
                "true_level2_element_count": (
                    mesh_audit["elements"] >= contract.minimum_fine_elements
                ),
                "positive_mesh_quality": mesh_audit["minimum_quality"] > 0.0,
                "fixed_40_by_140_mm_domain": domain
                == {
                    "r_domain": contract.radial_domain_mm,
                    "z_domain": contract.axial_domain_mm,
                },
            }
        )
        model.save(str(MODEL_PATH))
        return {
            "schema_version": "h11_corrected_gas_fine_continuation_audit_v1",
            "status": (
                "pass_corrected_gas_fine_continuation"
                if all(gates.values())
                else "fail_corrected_gas_fine_continuation_gates"
            ),
        ...
        failure = {
            "schema_version": "h11_corrected_gas_fine_continuation_failure_v1",
            "status": "failed_corrected_gas_fine_continuation_solve",
            "contract": str(CONTRACT_PATH.resolve()),
            "contract_sha256": _sha256(CONTRACT_PATH),
            "source_model": str(GAS_SKELETON_MODEL.resolve()),
            "source_model_sha256": _sha256(GAS_SKELETON_MODEL),

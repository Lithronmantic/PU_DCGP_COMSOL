"""H11 layer 11h: localize the residual that blocks strict steady convergence.

The torch-face correction removes the dominant nonphysical upstream inflow,
but a 1e-6 outer solution-and-residual solve still reaches 4000 segregated
iterations.  This diagnostic starts from that bounded last iterate, performs
only a few frozen outer updates, stores the latest assembled residual in the
solution, and requests COMSOL's detailed field-wise convergence log.

No geometry, mesh, physics, material, boundary condition, or calibration
parameter is changed.  Raw residual fields are used only to locate a stalled
equation spatially; their units differ and they must not be ranked across
fields.  Field-wise scaled error ranking comes from COMSOL's detailed log.
"""
...
class ResidualLocalizationContract:
    """Frozen numerical-only residual localization settings."""
...
def configure_residual_localization(
    ...
    *,
    ...
    """Build the short residual-storage branch without changing physics."""
...
def parse_detailed_residual_log(text: str) -> dict[str, Any]:
    """Extract the last scaled group and field residual estimates."""

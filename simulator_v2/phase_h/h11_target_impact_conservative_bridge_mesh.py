"""H11 layer 11d: solve a log-spaced bridge mesh between levels 3 and 2.

COMSOL's automatic levels 3 and 2 contain 12,738 and 40,488 elements in this
model, respectively.  This runner constructs a deterministic fractional step
in logarithmic global element-size space while retaining the audited local
nozzle and boundary-layer controls.  The converged level-3 all-Mach solution
is used only as the initial iterate; the conservative equations are resolved
on the bridge mesh.
"""
...
class BridgeMeshContract:
    """Frozen numerical definition of the level-3-to-level-2 bridge."""
...
def _fraction_token(fraction: float) -> str:
    """Return a stable artifact token for a bridge fraction."""

"""H11: strict fully coupled refinement of the nominal A-group free jet.

The accepted nominal free-jet field is stable at the fixed 100 mm diagnostic
plane, but its segregated solver does not satisfy COMSOL's scaled pressure
residual criterion.  This numerical-only branch keeps geometry, mesh,
physics, material data, and boundary conditions fixed.  It replaces the
active segregated nonlinear attribute with a residual-controlled, fully
coupled damped Newton solve using the existing PARDISO direct solver.

The branch is not calibration and cannot authorize a DPV prediction.  Its
sole purpose is to determine whether the remaining pressure residual is a
segregated-solver artifact.
"""
...
from simulator_v2.phase_h.h11_conservative_free_jet_residual_audit import (
    fixed_dpv_relative_changes,
)
from simulator_v2.phase_h.h11_target_impact_conservative_fully_coupled_refinement import (
...
class FreeJetFullyCoupledContract:
    """Frozen near-solution Newton and observable-stability settings."""

    relative_tolerance: float = 1e-6
    # The 100-iteration prospective run reduced the scaled pressure residual
    # from 6.3 to 0.0087 without a terminal plateau.  The frozen 300-iteration
    # budget follows from the observed iterations 90--100 contraction rate.
...
def assess_numerical_result(
    *,
    ...
    """Keep strict convergence separate from target-observable stability."""
...
def refinement_study_tag(source_study_tag: str) -> str:
    """Select a distinct tag so a converged strict solution can be verified."""

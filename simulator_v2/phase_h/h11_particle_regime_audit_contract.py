"""Regime audit required before releasing the conservative 7YSZ particle model.

The old particle skeleton used a constant Nusselt number of two and COMSOL's
single-temperature particle state.  Those assumptions are not accepted by
default in the A-group branch:

* Nu=2 is the zero-relative-velocity limit, while DPV particles have finite
  slip relative to the plasma;
* COMSOL's built-in particle temperature is spatially uniform and therefore
  requires a small Biot number;
* high-temperature plasma-particle momentum and heat transfer can require
  compressibility, variable-property, and rarefaction corrections.

This module freezes the dimensionless-number definitions and decision rules.
It does not invent gas states before the conservative free-jet solution exists,
and the Ranz-Marshall expression is used only as a screening estimate for the
Biot audit, not as a released plasma heat-transfer closure.
"""
...
class ParticleRegimeAuditContract:
    """Frozen thresholds and scientific claim boundary for regime selection."""
...
class ParticleRegimeSample:
    """One gas-particle state sampled along an accepted trajectory."""
    ...
    def screening_nusselt(self) -> float:
        """Ranz-Marshall screening value, not the final plasma closure."""

        return 2.0 + 0.6 * math.sqrt(self.reynolds) * self.prandtl ** (1.0 / 3.0)

    @property
    def screening_biot(self) -> float:
        """Sphere Bi using characteristic length V/A=d/6."""
...
def audit_particle_regimes(
    ...
    """Aggregate trajectory samples into model-selection decisions."""

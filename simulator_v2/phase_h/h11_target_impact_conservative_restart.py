"""H11 layer 11b: solve final all-Mach RANS from a bounded full-load field.

The source weakly-compressible solution supplies only the nonlinear initial
guess on the same geometry and mesh.  The final solve replaces its physics
with COMSOL's conservative all-Mach k-omega interface and recomputes the full
load state.  No source residual, source flux, or source prediction is accepted
as a final result.
"""
...
class ConservativeRestartContract:
    """Frozen numerical restart assumptions; none is calibration evidence."""
...
def replace_with_conservative_rans(
    ...
    """Replace source physics and seed final fields through withsol()."""
